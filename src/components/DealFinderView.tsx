...
export default function DealFinderView({ products, onAddProduct, setActiveView, language }: DealFinderViewProps) {
  ...
  const [newImageUrl] = useState('https://images.unsplash.com/photo-1584269600464-37b1b58a9fe7?w=400&auto=format&fit=crop&q=80');
  ...
  const handleAddSubmit = (e: React.FormEvent) => {
    ...
    const newDeal: ProductDeal = {
      ...
      title: newTitle || 'New Mined Product',
      category: newCategory,
      imageUrl: newImageUrl,
      supplierName: newSupplier || 'US Wholesale Supplier',
    ...
    <div className="space-y-6 select-none">
      ...
      <div className="p-4 bg-slate-900/90 border border-slate-800 rounded-xl grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="relative">
          <label className="text-[10px] font-mono text-slate-400 uppercase mb-1 block">{t.searchLabel}</label>
          <div className="relative">
        ...
        {/* Category */}
        <div>
          <label className="text-[10px] font-mono text-slate-400 uppercase mb-1 block">{t.categoryLabel}</label>
          <select
        ...
        <div className="flex items-center justify-end">
          <div className="text-right text-[11px] text-slate-400 font-mono">
            <div>Prep Fee: <strong className="text-white">$1.50/unit</strong></div>
            <div>Referral Fee: <strong className="text-white">15%</strong></div>
          </div>
        </div>
      </div>
          ...
          <table className="w-full text-left text-xs text-slate-300">
            <thead className="bg-slate-950 text-slate-400 uppercase text-[10px] tracking-wider border-b border-slate-800 font-mono">
              <tr>
                ...
                <th className="py-3.5 px-4">{t.colSupplier}</th>
                <th className="py-3.5 px-4">{t.colPricing}</th>
                <th className="py-3.5 px-4">{t.colFees}</th>
                <th className="py-3.5 px-4">{t.colProfitRoi}</th>
                <th className="py-3.5 px-4">{t.colUngated}</th>
              ...
              {filteredProducts.map((p) => (
                <tr key={p.id} className="hover:bg-slate-800/40 transition">
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div className="space-y-1">
                        ...
                        <div className="flex items-center space-x-2 text-[10px] font-mono text-slate-400">
                          ...
                          <span>•</span>
                          <span>Est. {p.estMonthlySales} sales/mo</span>
                        </div>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="py-4 px-4 font-mono">
                    <div className="text-slate-300">Buy: <strong className="text-white">${p.supplierPrice.toFixed(2)}</strong></div>
                    <div className="text-slate-400">Sell: <strong className="text-blue-400">${p.amazonPrice.toFixed(2)}</strong></div>
                  </td>
        ...
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-lg w-full p-6 space-y-5 shadow-2xl relative">
            ...
            <form onSubmit={handleAddSubmit} className="space-y-4 text-xs">
              <div>
                <label className="block text-slate-300 font-medium mb-1">{t.modalProductTitle}</label>
                <input
                  type="text"
                  required
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-slate-300 font-medium mb-1">{t.modalAsin}</label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label className="block text-slate-300 font-medium mb-1">{t.modalCategory}</label>
                  <select
                    value={newCategory}
              ...
              <div>
                <label className="block text-slate-300 font-medium mb-1">{t.modalSupplier}</label>
                <input
                  type="text"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-slate-300 font-medium mb-1">{t.modalBuyPrice}</label>
                  <input
                    type="number"
                    step="0.01"
                    required
                ...
                <div>
                  <label className="block text-slate-300 font-medium mb-1">{t.modalSellPrice}</label>
                  <input
                    type="number"
                    step="0.01"
                    required
